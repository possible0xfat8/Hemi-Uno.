...
export const LeaderboardModal: React.FC<LeaderboardModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-200">
      <div className="bg-[#0E1217] border border-slate-800 rounded-3xl max-w-xl w-full p-6 sm:p-8 shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="flex items-center justify-between pb-4 border-b border-slate-800/80 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-[#FF4600]/15 border border-[#FF4600]/30 flex items-center justify-center text-[#FF4600]">
              <Trophy className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-xl font-black text-white flex items-center gap-2">
                Leaderboard
                <span className="text-xs font-mono px-2 py-0.5 rounded-full bg-[#FF4600]/20 text-[#FF4600] border border-[#FF4600]/30 font-bold">
                  Global
                </span>
              </h2>
              <p className="text-xs text-slate-400">Top card champions and high earners on Hemi</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin text-[#FF4600]' : ''}`} />
            </button>
            <button
              onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-2.5 pr-1">
          {loading ? (
            <div className="py-16 text-center text-slate-500 font-mono text-xs flex flex-col items-center gap-3">
              <RefreshCw className="w-6 h-6 animate-spin text-[#FF4600]" />
